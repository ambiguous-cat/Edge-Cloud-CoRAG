...
class RAGChatService:
    ...
    def retrieve_documents(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if reranker.is_rerank_enabled():
                candidate_k = min(final_top_k * reranker.get_candidate_multiplier(), reranker.get_max_candidates())
                print(f"DEBUG: 重排序已启用，粗召回数量: {candidate_k}，最终返回数量: {final_top_k}")

            results = self.searcher.search_similar_documents(query, candidate_k)

            if reranker.is_rerank_enabled() and results:
                try:
                    return reranker.rerank(query, results, top_n=final_top_k)
                except Exception as rerank_error:
                    print(f"重排序失败，降级使用FAISS结果: {rerank_error}")
    ...
    def filter_documents_by_similarity(self, documents: List[Dict[str, Any]], similarity_threshold: float = 0.0) -> \
    Dict[str, Any]:
        """
        根据相似度阈值过滤文档
        ...
        try:
            for chunk in chat_model.stream_chat(messages, **kwargs):
                ...
                if not first_token_received and chunk.strip():
                    ...
                    total_time_to_first_token = first_token_time - start_time
                    print(f"DEBUG: rag_chat第一个token延迟: {first_token_delay:.2f}秒")
                    print(f"DEBUG: 首字响应时间（从开始到第一个token）: {total_time_to_first_token:.2f}秒")
            ...
            print(f"DEBUG: rag_chat总耗时: {total_time:.2f}秒")
    ...
    def rag_chat_stream_with_info(self, query: str, top_k: int = 3,
        ...
        filter_time = time.time() - filter_start
        print(
            f"DEBUG: 文档过滤耗时: {filter_time:.3f}秒，过滤前{filter_result['original_count']}个，过滤后{filter_result['accepted_count']}个")
        ...
        for i, msg in enumerate(messages):
            content_preview = msg.get('content', '')[:200] + '...' if len(msg.get('content', '')) > 200 else msg.get(
                'content', '')
        ...
        try:
            for chunk in chat_model.stream_chat(messages, **kwargs):
                ...
                if not first_token_received and chunk.strip():
                    ...
                    first_token_delay = first_token_time - model_start
                    total_time_to_first_token = first_token_time - start_time
                    print(f"DEBUG: 模型推理第一个token延迟: {first_token_delay:.2f}秒")
                    print(f"DEBUG: 首字响应时间（从开始到第一个token）: {total_time_to_first_token:.2f}秒")
                    first_token_received = True
            ...
            end_time = time.time()
            total_time = end_time - start_time
            print(f"DEBUG: rag_chat_with_info总耗时: {total_time:.2f}秒")
            ...
            yield {
                ...
                "timing_stats": {
                    ...
                    "first_token_response_time": response_time
                }
            }
...
def create_rag_service(model_type: str = "glm-4.6") -> RAGChatService:
    """创建RAG服务实例"""
...
if __name__ == "__main__":
    ...
    for question in test_questions:
        print(f"\n{'=' * 50}")
        rag_ask_stream(question, model_type="glm-4.6", top_k=2)
        print("-" * 50)
